...
public class Integers {
	public static void main(String[] args) {
		println(headTailAscOrder(0, 10,6));
		println(headAscOrder(0, 10,6));
		println(tailAscOrder(0, 10,6));
		println(centerAscOrder(0, 10,6));
		println(random(5, 0,10));
		println(same(10,3));
	}
	//从min到max随机生成count个数
	...
	//返回一个长度为count的数组，前unsameCount位是unsameCount到1，后面都是unsameCount+1
	...
	//返回一个长度为count的数组且头尾升序，中间disorderCount位无序
	...
	//从min到max中返回一个长度为max - min + 1的数组且头尾升序，中间disorderCount位无序
	...
	//从min到max中返回一个长度为max - min + 1的数组且头部升序，后disorderCount位无序
	...
	//从min到max中返回一个长度为max - min + 1的数组且尾部升序，前disorderCount位无序
	...
	//从min到max中返回一个长度为max - min + 1的升序数组
	...
	//从min到max中返回一个长度为max - min + 1的降序数组
